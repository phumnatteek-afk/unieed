...
export default function ProjectDetailPage() {
  ...
  const [activeType, setActiveType]     = useState(null);     // uniform_type_id | null = all
  ...
  const itemKey = (item) => {
    const idx = project?.uniform_items?.indexOf(item);
    return `item_${idx ?? 0}`;
  };
  ...
  const UniformBlock = () => {
    ...
    // levels
    const levels = [...new Set(allItems.map(i => i.education_level).filter(Boolean))];
    ...
    // genders ที่มี
    const hasMale   = levelItems.some(i => genderOf(i.name) === "male"   || genderOf(i.name) === "all");
    const hasFemale = levelItems.some(i => genderOf(i.name) === "female" || genderOf(i.name) === "all");

    // กรองตาม gender
    const genderItems = levelItems.filter(i => {
      const g = genderOf(i.name);
      return activeGender === "male" ? (g === "male" || g === "all") : (g === "female" || g === "all");
    });

    // uniform types สำหรับ tab ประเภท
    const typeMap = new Map();
    genderItems.forEach(i => {
      typeMap.set(i.uniform_type_id, i.name.split(" ")[0]);
    });
    const types = [...typeMap.entries()]; // [[id, label], ...]

    // set default type ถ้ายังไม่ได้เลือกหรือ type ปัจจุบันไม่มีใน filter ใหม่
    const validTypeIds = types.map(([id]) => id);
    const currentType = validTypeIds.includes(activeType) ? activeType : (validTypeIds[0] ?? null);

    // กรองตาม type
    const typeItems = currentType
      ? genderItems.filter(i => i.uniform_type_id === currentType)
      : genderItems;

    // รูปสำหรับ type ที่เลือก (dedup)
    const thumbItems = (() => {
      const seen = new Set();
      return genderItems.filter(i => {
        if (i.image_url && !seen.has(i.uniform_type_id)) { seen.add(i.uniform_type_id); return true; }
        return false;
      });
    })();
        ...
        <div className="pdGenderTabs">
          {hasMale && (
            <button
              ...
              onClick={() => { setActiveGender("male"); setActiveType(null); }}
            >
 ...
 เพศชาย
          ...
          {hasFemale && (
            <button
              ...
              onClick={() => { setActiveGender("female"); setActiveType(null); }}
            >
             <svg width="25" height="25" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M11.459 19.792H10.4173C10.1222 19.792 9.87496 19.692 9.67565 19.492C9.47635 19.292 9.37635 19.0448 9.37565 18.7503C9.37496 18.4559 9.47496 18.2087 9.67565 18.0087C9.87635 17.8087 10.1236 17.7087 10.4173 17.7087H11.459V15.5212C10.0875 15.2781 8.96315 14.6225 8.08607 13.5545C7.20899 12.4864 6.77079 11.2496 6.77149 9.84408C6.77149 8.26421 7.33155 6.92324 8.45169 5.82116C9.57183 4.71908 10.9215 4.16769 12.5007 4.16699C14.0798 4.1663 15.4298 4.71769 16.5507 5.82116C17.6715 6.92463 18.2312 8.2656 18.2298 9.84408C18.2298 11.2503 17.7913 12.4875 16.9142 13.5555C16.0371 14.6236 14.9132 15.2788 13.5423 15.5212V17.7087H14.584C14.8791 17.7087 15.1267 17.8087 15.3267 18.0087C15.5267 18.2087 15.6263 18.4559 15.6257 18.7503C15.625 19.0448 15.525 19.2923 15.3257 19.493C15.1263 19.6937 14.8791 19.7934 14.584 19.792H13.5423V20.8337C13.5423 21.1288 13.4423 21.3764 13.2423 21.5764C13.0423 21.7764 12.7951 21.876 12.5007 21.8753C12.2062 21.8746 11.959 21.7746 11.759 21.5753C11.559 21.376 11.459 21.1288 11.459 20.8337V19.792ZM15.0788 12.4743C15.7906 11.7625 16.1465 10.9031 16.1465 9.89616C16.1465 8.88922 15.7906 8.02984 15.0788 7.31803C14.367 6.60623 13.5076 6.25033 12.5007 6.25033C11.4937 6.25033 10.6343 6.60623 9.92253 7.31803C9.21072 8.02984 8.85482 8.88922 8.85482 9.89616C8.85482 10.9031 9.21072 11.7625 9.92253 12.4743C10.6343 13.1861 11.4937 13.542 12.5007 13.542C13.5076 13.542 14.367 13.1861 15.0788 12.4743Z" fill="currentColor"/>
 ...
 เพศหญิง
          ...
          <div className="pdLevelTabs">
            {levels.map(lv => (
              <button
                ...
                onClick={() => { setActiveLevel(lv); setActiveType(null); }}
              >
                {lv}
              </button>
            ))}
        ...
        {types.length > 1 && (
          <div className="pdTypeTabs">
            {types.map(([id, label]) => (
              <button
                key={id}
                className={`pdTypeTab ${currentType === id ? "pdTypeTabActive" : ""}`}
                onClick={() => setActiveType(id === currentType ? null : id)}
              >
                {label}
              </button>
            ))}
        ...
        {/* ── รูปชุด (แสดงทุกตัวของ gender+level ที่เลือก) ── */}
        <div className="pdUniformImgs">
          {thumbItems.map(item => (
            <div key={item.uniform_type_id} className="pdUniformImgWrap">
              <div
                className={`pdUniformImgBtn ${currentType === item.uniform_type_id ? "pdUniformImgBtnActive" : ""}`}
                onClick={() => setActiveType(prev => prev === item.uniform_type_id ? null : item.uniform_type_id)}
              >
